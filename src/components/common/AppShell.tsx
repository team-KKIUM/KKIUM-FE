...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const showProtectedContent = hideSidebar || authReady;

  return (
    <div
      style={appShellStyle}
      className="min-h-dvh bg-background-default"
      aria-busy={!hideSidebar && !authReady}
    >
      {!hideSidebar && authReady && <Sidebar collapsed={collapsed} />}
      <main
        className={`ml-(--app-sidebar-width) min-h-dvh min-w-0 ${hideSidebar ? 'pt-0' : 'pt-[30px]'}`}
      >
        <div className={hideSidebar ? undefined : 'mx-20'}>
          {showProtectedContent ? children : null}
        </div>
      </main>
    </div>
  );
